...
class SSFM:
    ...
    def __init__(self,  local_error = 0.001, dz = 1e-5,
                 ...
                 suppress_iteration = True, USE_SIMPLE_RAMAN = False,USE_MVM_RAMAN=False,
        ...
        self.method = SSFM.METHOD_SSFM#SSFM.METHOD_RK4IP
    ...
    def load_fiber_parameters(self, pulse_in, fiber, output_power, z=0):
        ...
        self.fibertype = fiber.fibertype

        if self.fibertype == 'gainfiber':
            self.g0 = fiber.gain
            self.Esat = fiber.Esat
            self.Omg = fiber.Omg
            self.ome_gg = self.Omg*2*np.pi*pulse_in._c_nmps/pulse_in.center_wavelength_nm**2
            self.dT = pulse_in.dT_mks
    ...
    def setup_fftw(self, pulse_in, fiber, output_power, raman_plots = False):
        ...
        self.wl_nm     = pulse_in.wl_nm
        self.center_wavelength_nm=pulse_in.center_wavelength_nm
...
#                printf("[%d] dz = %f\n",self.iter,dz)
    ...
    def LinearStep(self,A,h,direction):
        if h!=self.last_h or direction!=self.last_dir or self.last_h is None:
            self.Calculate_expD(A,h,direction)
            self.last_h = h
 ...
 #        else:
    ...
    def Calculate_expD(self,A,h,direction):
        if self.fibertype != 'gainfiber':        
            self.exp_D[:] = np.exp(direction*h*0.5*(1j*self.betas-self.alpha/2.0))
        else:
            g = self.g0/(1+np.sum(np.abs(A)**2)*self.dT/self.Esat)
            self.exp_D[:] = np.exp(direction*h*0.5*(1j*self.betas+0.5*(g-self.alpha)\
                -(self.omegas)**2*g/(2*self.ome_gg**2)))
    def filter(self, pulse_in,band,fiber,output_power=None):
        ...
        self.T_BP=np.exp(-(self.wl_nm-self.center_wavelength_nm)**2/2/self.band0**2)
